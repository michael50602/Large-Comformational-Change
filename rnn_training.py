...
def fill_mx(max_len, feature, protein_prop):
    dense_mx = []
...
def run(feature):
    ...
    dense_mx = fill_mx(max_sequence_len, feature, protein_prop)
    print dense_mx
    pred = RNN(x, weight, bias, n_hidden, feature, split_size)
    cost  = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(pred, y, 1/50))
